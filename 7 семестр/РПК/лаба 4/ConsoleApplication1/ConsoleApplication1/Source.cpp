...
int main()
{
	...
	double Ua = 10.; 
	double Ub = 20.;//граничные условия
	double K_loc[3][3] = { { 1.,-1. },{ -1.,1. } };

	double** Elems = new double*[2 * N*N];
	...
	for (int i = 0; i < N; i++)
		for (int j = 0; j < N; j++)
		{
			Elems[2 * (i * N + j)][0] = i*N + j + 1;
			Elems[2 * (i * N + j)][1] = (i + 1)*N + j + 1;
			Elems[2 * (i * (N - 1) + j)][2] = (i + 1)*N + j + 1 + 1;
			Elems[2 * (i * (N - 1) + j) + 1][0] = i*N + j + 1;
			Elems[2 * (i * (N - 1) + j) + 1][1] = (i + 1)*N + j + 1 + 1;
			Elems[2 * (i * (N - 1) + j) + 1][2] = i*N + j + 1 + 1;
		}
	...
	{
		K[i] = new double[n*n];
		for (int j = 0; j < n*n; j++) K[i][j] = 0.;
	}

	double* F = new double[n*n];
	for (int j = 0; j < n*n; j++) F[j] = 0.;
	...
	for (int i = 0; i < N; i++) delete[] Elems[i];
